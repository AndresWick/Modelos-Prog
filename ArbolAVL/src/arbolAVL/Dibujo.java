...
public class Dibujo extends JPanel {
     ...
     * dibujar.
     */
    public Dibujo(ArbolAVL miArbol) {
    ...
    private void calcularPosiciones() {
         posicionNodos.clear();
         subtreeSizes.clear();
         NodoAVL root = this.miArbol.getRaiz();
         if (root != null){
    ...
    private Dimension calcularTamañoSubarbol(NodoAVL n){
          ...
          Dimension ld = calcularTamañoSubarbol(n.getHijoIzq());
          Dimension rd = calcularTamañoSubarbol(n.getHijoDer());
    ...
    private void calcularPosicion(NodoAVL n, int left, int right, int top) {
      ...
      Dimension ld = (Dimension) subtreeSizes.get(n.getHijoIzq());
      if (ld == null) 
          ld = empty;
      
      Dimension rd = (Dimension) subtreeSizes.get(n.getHijoDer());
      if (rd == null) 
          rd = empty;
      ...
      calcularPosicion(n.getHijoIzq(), Integer.MAX_VALUE, center - child2child/2, top + fm.getHeight() + parent2child);
      calcularPosicion(n.getHijoDer(), center + child2child/2, Integer.MAX_VALUE, top + fm.getHeight() + parent2child);
    }
    ...
    // g.draw(r);
     ...
     dibujarArbol(g, n.getHijoIzq(), (int)(r.x + r.width/2), r.y + r.height, yoffs);
     dibujarArbol(g, n.getHijoDer(), (int)(r.x + r.width/2), r.y + r.height, yoffs);
